...
// Note: KOS doesn't define these macros and has no implementation of flock()
// TODO: replace this function with the proper flock() implementation/call.
extern "C" {
# define LOCK_SH 1
# define LOCK_EX 2
# define LOCK_NB 4
# define LOCK_UN 8
int flock_kos(int fd, int operation) {
    struct flock f;
...
Status StorageEngineLockFile::open() {
    ...
    int ret = flock_kos(lockFile, LOCK_EX | LOCK_NB);
    if (ret != 0) {
        auto ec = lastPosixError();
        ::close(lockFile);
        return Status(ErrorCodes::DBPathInUse,
...
void StorageEngineLockFile::close() {
    ...
    flock_kos(_lockFileHandle->_fd, LOCK_UN);
