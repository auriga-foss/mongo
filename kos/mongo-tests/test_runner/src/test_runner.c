...
static const char* retcodeToStr(Retcode code) {
    switch (code) {
        case rcOk:
        return "rcOk";
...
static Retcode testElfLoadFromDisk(const char* path, rtl_uint8_t** img_ptr, rtl_uint64_t* imgSize) {
	...
	if ((fstat(fd, &file_stat_buf) != 0) || (!S_ISREG(file_stat_buf.st_mode))) {
        fprintf(stderr, "fstat error for %s file\n", path);
        close(fd);
        return rcFail;
	}
    
    file_size = (rtl_uint64_t) file_stat_buf.st_size;
...
static Retcode testRun(const EntityInfo *einfo, const char *path) {
    Entity *entity = RTL_NULL;
    Retcode rc = rcOk;
    rtl_uint8_t *img = RTL_NULL;
    rtl_uint64_t imgSize = 0;

    fprintf(stderr, "Test " TEST_NAME " loading...\n", path);

    rc = testElfLoadFromDisk(path, &img, &imgSize);
    if (rc == rcOk) {
        rtl_uintptr_t entry;
        VmSeg *segs = RTL_NULL;
        ...
        rc = KnElfCreateVmSegEx(img, imgSize, &entry,
                                &segs, &segsCount,
                                &symIndex, &symSize,
                                &strIndex, &strSize,
                                &hdrData, &hdrSize);
        ...
        if (rc == rcOk) {
            const char *taskName = einfo->eiid;
            ...
            if (rc == rcOk) {
                ...
                fprintf(stderr, "Test "ANSI_COLOR_BLUE"%s"ANSI_COLOR_RESET" started!\n", path);
            
                rc = EntityWait(entity, INFINITE_TIMEOUT, &entity_rc);

                if (rc == rcOk) {
                    char* test_result = (entity_rc == 0) ? TEST_SUCCESS : TEST_FAIL;    
                    fprintf(stderr, "Test "ANSI_COLOR_BLUE"%s"ANSI_COLOR_RESET" completed: %s\n", path, test_result);
                } else {
                    fprintf(stderr,"testRun: EntityWait return code: %s for %s\n", retcodeToStr(rc), path);
                }
            } else {
...
#define FILE_PATH_1 "/tests/db_sorter_test"
#define FILE_PATH_2 "/tests/db_bson_test"


/* 
    Arguments:
     - Path to directory where test executables are located
 */
